...
export default function ScenarioDetail({ worldId, scenario, onClose }) {
	...
	const handleResimulate = async () => {
		...
		try {
			await api.post(
				`/small-world/worlds/${worldId}/scenarios/${scenarioId}/run`,
			);
			setReport(null);
			setTab('stream');
	...
	// Load report when: tab is 'report' AND sim is not running (completed or idle)
	useEffect(() => {
		if (tab !== 'report' || !scenarioId || !worldId) return;
		if (liveStatus === 'running') return; // sim in progress — nothing to show yet
		if (report) return;
				...
				>
						...
						>
							<span
								style={{
									...
									borderRadius: '50%',
									background:
										STATUS_COLOR[liveStatus] || '#94a3b8',
					...
					>
						<button
							...
							disabled={resimulating || liveStatus === 'running'}
							title="Resimulate"
							style={{
								display: 'flex',
								alignItems: 'center',
